...
1000 × 1000 reference images.

1000 × 1000 clean search images.

Defective/degraded search images.

Ground-truth target-location information.

Per-pair metadata.

Random seed and transformation/degradation parameters.
...
Loads and validates both images.

Removes the artificial reference-image border where required.

Explicitly searches across the 9:1–11:1 scale range.

Downsamples the reference to candidate search-image scales.

Performs normalized cross-correlation (NCC).

Detects multiple local maxima rather than using only one global maximum.

Removes duplicate detections belonging to the same spatial occurrence.

Keeps strong, spatially distinct candidate locations.

Selects the candidate closest to the search-image centre.

Reports the predicted target centre, score, scale and runtime.

Saves a visualization of the detected locations.
...
Finds the strongest NCC response.

Keeps candidates within the configured score window of the best response.

Suppresses duplicate detections belonging to the same physical location.

Retains multiple distinct candidate locations.

Computes each candidate's distance from the centre of the search image.

Selects the valid candidate with minimum centre distance.
